...
const blankUnit = (): EditableUnit => ({ id: uid(), ucNumber: '', holderName: '', address: '', basisPoints: 0, locked: false, origin: 'new' });
...
export default function RateioForm({ initialReference = '' }: { initialReference?: string }) {
  ...
  const validUnits = units.length > 0 && units.length <= 20 && units.every((unit) => /^\d{15}$/.test(unit.ucNumber) && unit.address.trim() && unit.basisPoints > 0);
  ...
  function openManual() {
    setManual(true); setProject({ ...emptyProject(), reference }); setFee({ status: 'indeterminate' }); setUnits([blankUnit()]); setOriginalUnits([]); setMountedAt(Date.now()); setFormDirty(false); setStage('form'); setMessage('Não foi possível confirmar automaticamente o projeto. Preencha todos os dados; a equipe fará uma conferência manual.');
  }

  function confirmLookup() {
    if (!lookup) return;
    const imported = toUnits(lookup); setUnits(imported); setOriginalUnits(imported.map((unit) => ({ ...unit }))); setMountedAt(Date.now()); setFormDirty(false); setStage('form'); setMessage('');
  }
  ...
  function addUnit() { if (units.length < 20) { setFormDirty(true); setUnits((current) => [...current, blankUnit()]); } }
  function removeUnit(id: string) { setFormDirty(true); setUnits((current) => { const remaining = current.filter((unit) => unit.id !== id); return remaining.length ? remaining : [blankUnit()]; }); }
  ...
  async function submit(event: React.FormEvent) {
    ...
    const shareUnits = units.map((unit) => ({ ucNumber: unit.ucNumber, holderName: unit.holderName || project.holder.name, address: unit.address || null, percent: unit.basisPoints / 100 }));
    try {
      const response = await fetch('/api/rateio/solicitacoes', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ manual, website, mountedAt, lookupToken: lookup?.lookupToken, requestType: inferRequestType(units, originalUnits), expectedFeeStatus: fee?.status, project, payload: { shareUnits, observations, consent } }) });
      const data = await response.json();
  ...
  function reset() { setStage('lookup'); setReference(initialReference); setVerifier(''); setLookup(null); setManual(false); setProject(emptyProject()); setUnits([]); setOriginalUnits([]); setFee(null); setConsent(false); setFeeAccepted(false); setObservations(''); setMessage(''); setProtocol(''); setFailures(0); setShowErrors(false); setFormDirty(false); }
  ...
  const showTotal = showErrors || units.some((unit) => unit.basisPoints > 0);
  const changed = !unitsMatch(units, originalUnits);
  ...
  return (
    <form noValidate onSubmit={submit} className="rateio-form min-w-0 space-y-5 pb-32 sm:pb-36 md:pb-0">
      ...
      <section className="min-w-0 border-b border-slate-200 bg-white px-1 pb-5">
        ...
        {manual ? <ManualProject project={project} showErrors={showErrors} onChange={(value) => { setFormDirty(true); setProject(value); }} /> : (
      ...
      <section className="min-w-0 bg-white px-1 py-2">
        ...
        <div className="mt-4 space-y-3">
          {units.map((unit, index) => (
            <div key={unit.id} className="grid min-w-0 scroll-mt-28 gap-3 border-b border-slate-200 pb-4 md:grid-cols-[1fr_1.6fr_180px_44px] md:items-end">
              ...
              <label data-rateio-error={showErrors && unit.basisPoints <= 0 ? 'true' : undefined} className="text-sm font-semibold">{project.state === 'GO' ? 'Percentual do excedente' : 'Percentual da geração'}
                <input inputMode="decimal" aria-label={`Percentual da unidade ${unit.ucNumber || index + 1}`} key={`${unit.id}-${unit.basisPoints}`} defaultValue={unit.origin === 'new' && unit.basisPoints === 0 ? '' : formatPercent(unit.basisPoints)} onBlur={(event) => updatePercent(unit.id, event.target.value)} className="rateio-control mt-1 w-full min-w-0 rounded-lg border border-slate-300 px-3 py-2" />
                {showErrors && unit.basisPoints <= 0 && <span role="alert" className="mt-1 block text-xs font-semibold text-red-700">Informe um percentual maior que zero.</span>}
              </label>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {changed && originalUnits.length > 0 && <button type="button" onClick={() => { setFormDirty(true); setUnits(originalUnits.map((unit) => ({ ...unit }))); }} className="border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700"><RotateCcw className="mr-1 inline h-4 w-4" />Voltar à distribuição atual</button>}
          {units.length > 1 && <button type="button" onClick={() => { setFormDirty(true); setUnits((current) => redistribute(current, true)); }} className="border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700">Dividir igualmente entre as unidades</button>}
      ...
      <section className="min-w-0 border-t border-slate-200 bg-white px-1 pt-5">
        ...
        <ul className="mt-3 space-y-1 text-sm text-slate-700">{units.map((unit, index) => <li key={unit.id}>Unidade {unit.ucNumber || index + 1}: <strong>{formatPercent(Math.max(0, unit.basisPoints))}%</strong></li>)}</ul>
